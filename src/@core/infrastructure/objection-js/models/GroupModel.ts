    ...
    const entity = Group.create(
      {
        ...
        typeReport: dbJson["type_report"],
      },
      Number(dbJson["id"])
    );
    const term =
      dbJson["term"] && TermModel.convertModelToEntity(dbJson["term"]);
    const topic =
      dbJson["topic"] && TopicModel.convertModelToEntity(dbJson["topic"]);
    const members =
      dbJson["members"] &&
      dbJson["members"].map((e: any) =>
        GroupMemberModel.convertModelToEntity(e)
      );
