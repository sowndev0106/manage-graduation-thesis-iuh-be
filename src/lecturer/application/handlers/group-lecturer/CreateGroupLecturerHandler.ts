...
export default class CreateGroupLecturerHandler extends RequestHandler {
  ...
  private groupMemberDao!: IGroupLecturerMemberDao;
  ...
  async validate(request: Request): Promise<ValidatedInput> {
    const name = this.errorCollector.collect("name", () =>
      SortText.validate({ value: request.body["name"] })
    );
    const type = this.errorCollector.collect("type", () =>
      TypeEvaluationValidate.validate({ value: request.body["type"] })
    );
    const termId = this.errorCollector.collect("termId", () =>
      EntityId.validate({ value: request.body["termId"] })
    );
    const lecturerIds = this.errorCollector.collect("lecturerIds", () =>
      EntityIds.validate({
        value: request.body["lecturerIds"],
        required: false,
      })
    );
  ...
  async handle(request: Request) {
    ...
    if (groupLecturer)
      throw new ErrorCode(
        ...
        `group name ${name}  already exists in student`
      );
