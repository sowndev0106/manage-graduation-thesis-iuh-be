...
export default class AssignDao extends AssignDaoCore implements IAssignDao {
  async isFinishTranscript(props: {
  ...
  async findOneExtends(props: {
    ...
    query.join(
      ...
      "group_lecturer.id",
      "=",
      "assign.group_lecturer_id"
    );
    query.join(
      "group_lecturer_member",
      "group_lecturer_member.group_lecturer_id",
      "=",
      "group_lecturer.id"
    );
    ...
    whereClause["group_lecturer_member.lecturer_term_id"] =
      props.lecturerTermId;
  ...
  async findByLecturer(props: {
    ...
    query.withGraphFetched(
      "[group_lecturer, group_lecturer.members,group_lecturer.members.lecturer_term, group_lecturer.members.lecturer_term.lecturer, group, group.[members,topic,members.student_term,members.student_term.student]]"
    );
    query.join(
      "group_lecturer",
      "group_lecturer.id",
    ...
    query.join(
      "lecturer_term",
      "group_lecturer_member.lecturer_term_id",
      "=",
      "lecturer_term.id"
    );
    query
  ...
  async findAll(props: {
    ...
    if (props.termId) {
      query
        ...
        .where({ "group_lecturer.term_id": props.termId });
    }
    if (props.groupLecturerId)
      whereClause["group_lecturer_id"] = props.groupLecturerId;
    if (props.type) whereClause["type_evaluation"] = props.type;
