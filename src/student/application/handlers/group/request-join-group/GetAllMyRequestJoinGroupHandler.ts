...
@injectable()
export default class GetAllMyRequestJoinGroupHandler extends RequestHandler {
	...
	async handle(request: Request) {
		...
		const inviteJoinGroup = await this.requestJoinGroupDao.findAllByStudentIdAndType(input.studentId!, TypeRquestJoinGroup.REQUEST_JOIN);
